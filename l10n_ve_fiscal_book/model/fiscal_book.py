...
class fiscal_book(orm.Model):
    ...
    def update_book_lines(self, cr, uid, fb_id, context=None):
        ...
        if fb_brw.iwdl_ids:
            ...
            for iwdl_brw in iwdl_obj.browse(cr, uid, iwdl_ids,
                ...
                doc_type = self.get_doc_type(cr, uid, iwdl_id=iwdl_brw.id, fb_id=fb_id, context=context)
                values = {
                    'iwdl_id': iwdl_brw.id,
                    'type': t_type,
        ...
        for inv_brw in self.browse(cr, uid, fb_id,
            ...
            values = {
                ...
                'credit_affected': inv_brw.parent_id and \
                                   ...
                                   and inv_brw.parent_id.number or False,
                'nro_ctrl': not inv_brw.fiscal_printer and inv_brw.nro_ctrl or False,
                'affected_invoice': (doc_type == "N/DB" or doc_type == "N/CR") \
    ...
    def get_doc_type(self, cr, uid, inv_id=None, iwdl_id=None, cf_id=None,
        ...
        if inv_id:
            ...
            iwdl_obj = self.pool.get('account.wh.iva.line')
            iwdl_brw = iwdl_obj.browse(cr, uid, iwdl_id)
            if fb_id and fb_brw.type == 'sale' and\
               iwdl_brw.invoice_id.date_invoice[5:-3] !=\
               iwdl_brw.retention_id.date_ret[5:-3]:
                res = 'AJST'
            else:
                res = 'RET'
        elif cf_id:
...
class fiscal_book_lines(orm.Model):
    ...
    _columns = {
        ...
        'nro_ctrl': fields.char(string='Invoice Control number', size=64,
                                   help=''),
