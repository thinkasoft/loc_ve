...
class account_wh_timbreci(osv.osv):
    ...
    _columns = {
        ...
        'timbreci_line_ids': fields.one2many('account.wh.timbreci.line', 'retention_id', 'Local withholding lines', readonly=True, states={'draft': [('readonly', False)]}, help="Invoices to will be made local withholdings"),
    ...
    def action_cancel(self, cr, uid, ids, context=None):
        ...
        self.clear_timbreci_line_ids(cr, uid, ids, context=context)
        return True
    ...
    def cancel_move(self,cr,uid,ids, *args):
        ...
        for ret in ret_brw:
            if ret.state == 'done':
                for ret_line in ret.timbreci_line_ids:
    ...
    def clear_timbreci_line_ids(self, cr, uid, ids, context=None):
        """ Clear lines of current withholding document and delete wh document
        information from the invoice.
        """
        context = context or {}
        wml_obj = self.pool.get('account.wh.timbreci.line')
        ...
        if ids:
            ...
            ai_ids and ai_obj.write(cr, uid, ai_ids, {'wh_timbre_id': False}, context=context)
    ...
    def action_confirm(self, cr, uid, ids, context=None):
        ...
        if context is None:
            context = {}
        obj = self.pool.get('account.wh.timbreci').browse(cr, uid, ids)
        total = 0
        for i in obj[0].timbreci_line_ids:
    ...
    def action_number(self, cr, uid, ids, *args):
        """ Generate sequence for empty number fields in account_wh_timbreci records
        """
        obj_ret = self.browse(cr, uid, ids)[0]
        if obj_ret.type == 'in_invoice':
            cr.execute('SELECT id, number '
                       'FROM account_wh_timbreci '
                       'WHERE id IN (' + ','.join(map(str, ids)) + ')')

            for (id, number) in cr.fetchall():
                if not number:
                    number = self.pool.get('ir.sequence').get(
                        cr, uid, 'account.wh.timbre.%s' % obj_ret.type)
                cr.execute('UPDATE account_wh_timbreci SET number=%s '
    ...
    def action_move_create(self, cr, uid, ids, context=None):
        ...
        context.update({'timbre_wh': True})
        for ret in self.browse(cr, uid, ids):
            for line in ret.timbreci_line_ids:
            ...
            if ret.timbreci_line_ids:
                for line in ret.timbreci_line_ids:
                    ...
                    self.write(cr, uid, [ret.id], {
                               'timbreci_line_ids': lines, 'period_id': period_id})
                    inv_obj.write(
                        cr, uid, [line.invoice_id.id], {'wh_timbre_id': ret.id})
        return True
    ...
    def _update_check(self, cr, uid, ids, context=None):
        ...
        for id in ids:
            ...
            for line in awm_brw.timbreci_line_ids:
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        context = context or {}
        ids = isinstance(ids, (int, long)) and [ids] or ids
        res = super(account_wh_timbreci,self).write(cr, uid, ids, vals, context=context)
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        """
        context = context or {}
        new_id = super(account_wh_timbreci, self).create(cr, uid, vals, context=context)
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        context = context or {}
        for timbre_brw in self.browse(cr, uid, ids, context=context):
            if timbre_brw.state != 'cancel':
                raise osv.except_osv(_("Invalid Procedure!!"),
                    _("The withholding document needs to be in cancel state to be deleted."))
            else:
                super(account_wh_timbreci, self).unlink(cr, uid, ids, context=context)
        return True
    ...
    def check_wh_lines(self, cr, uid, ids, context=None):
        """ Check that wh timbre has withholding lines"""
        context = context or {}
        ids = isinstance(ids, (int, long)) and [ids] or ids
        awm_brw = self.browse(cr, uid, ids[0], context=context)
        if not awm_brw.timbreci_line_ids:
            raise osv.except_osv(
                _("Missing Values !"),
                _("Missing Withholding Lines!"))
        return True

class account_wh_timbreci_line(osv.osv):

    def default_get(self, cr, uid, fields, context=None):
        """ Default for timbreci_context field
        """
        if context is None:
            context = {}
        data = super(account_wh_timbreci_line, self).default_get(cr,
                                                               uid, fields, context)
        self.timbreci_context = context
        return data

# TODO
# necesito crear el campo y tener la forma de calcular el monto del impuesto
# timbreci retenido en la factura

    _name = "account.wh.timbreci.line"
    ...
    _columns = {
        ...
        'retention_id': fields.many2one('account.wh.timbreci', 'Local withholding', ondelete='cascade', help="Local withholding"),
    ...
    _sql_constraints = [
        ('timbreci_fact_uniq', 'unique (invoice_id)',
         'The invoice has already assigned in local withholding, you cannot assigned it twice!')
    ]

    def onchange_invoice_id(self, cr, uid, ids, invoice_id, wh_loc_rate=0.1, context=None):
        ...
        if hasattr(self, 'timbreci_context') and ('lines' in self.timbreci_context):
            lines = [x[2] for x in self.timbreci_context['lines']]
        if not invoice_id:
            ...
            cr.execute('select retention_id from account_wh_timbreci_line where invoice_id=%s',
                (invoice_id,))
            ret_ids = cr.fetchone()
            if bool(ret_ids):
                ret = self.pool.get(
                    'account.wh.timbreci').browse(cr, uid, ret_ids[0], context)
                raise osv.except_osv('Assigned Invoice !',
                                     "The invoice has already assigned in local withholding code: '%s' !" % (ret.code,))

            total = res.amount_untaxed * wh_loc_rate / 100.0
